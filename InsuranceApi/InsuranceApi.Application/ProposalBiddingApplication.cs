...
namespace InsuranceApi.Application {
    public class ProposalBiddingApplication : IProposalBiddingApplication {
        ...
        public ProposalBiddingApplication(
        ...
        public async Task<ProposalReturnGravarEntity> AddAsync(ProposalBiddingEntity entity, int brokerUserId) {
            try {
                ...
                var parameterCoverage =
                    parameter.Where(x => x.AppParameterId.Equals((int)AppParameterEnum.ModalidadeTipoSeguroLicitacao)).ToList();

                var isCoverageBidding = parameterCoverage.Where(x => x.Value.Equals(entity.CodigoModalidade)).FirstOrDefault();
                if (isCoverageBidding == null)
                    throw new BusinessException("Não foi possível encontrar a modalidade 'licitacao'.");
                ...
                proposalEntity.TipoSeguro = TipoSeguroEnum.Licitacao;
                proposalEntity.DadosGarantia.ValorImportanciaSegurada = entity.ValorGarantia;
                proposalEntity.DadosGarantia.NumeroLicitacao = entity.NumeroLicitacao;
